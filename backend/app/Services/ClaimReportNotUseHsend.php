    ...
    public function applyFilters(Builder $query, Request $request): Builder
    {
        ...
        if ($Hsend !== null) {
            if (is_array($Hsend) && count($Hsend) > 0) {
                $query->whereIn('ri.HSEND', $Hsend);
            } else if (!is_array($Hsend) && $Hsend !== '') {
                $query->where('ri.HSEND', $Hsend);
            }
        }
        ...
        // ตอนนี้ กรอง type_affiliation ด้วยตนเอง
        $type_affiliation = $request->input('type_affiliation');
        if ($type_affiliation !== null) {
            if (is_array($type_affiliation) && count($type_affiliation) > 0) {
                $query->whereIn('hmainop.type_hos', $type_affiliation);
            } elseif (!is_array($type_affiliation) && $type_affiliation !== '') {
                $query->where('hmainop.type_hos', $type_affiliation);
            }
        }

        // และกรอง name_Ampur ด้วยตนเอง
        $name_Ampur = $request->input('name_Ampur');
        if ($name_Ampur !== null) {
            if (is_array($name_Ampur) && count($name_Ampur) > 0) {
                $query->whereIn('hmainop.name_Ampur', $name_Ampur);
            } else if (!is_array($name_Ampur) && $name_Ampur !== '') {
                $query->where('hmainop.name_Ampur', $name_Ampur);
            }
        }
