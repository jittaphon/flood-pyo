...
export default function DMIS_CKD() {
  ...
  const [list_hamin, setlist_hamin] = useState([]); // ✅ สร้าง state สำหรับ hmainOP
   ...
   const fetchAppointments = useCallback(async () => {
    ...
    try {
      ...
      const responsedSummary = await API.DMIS_CKD.getDMIS_CKD_Summary(requestParams, options); 
      const responsedeatailData = await API.DMIS_CKD.getDMIS_CKD_Detail(requestParams, options);
      const ListNameType = await API.DMIS_CKD.getDMIS_CKD_ListNameType(options);  // <-- ส่ง options ไปด้วย
      const list_hmainOP = await API.DMIS_CKD.getDMIS_CKD_LlistHaminaOP(options); // <-- ส่ง options ไปด้วย
...
const handleAmpurChange = (newSelectedAmpur) => {
  ...
  return (
    <div className="p-6 bg-gradient-to-br from-blue-400 via-blue-100 to-blue-200 min-h-screen relative">
      ...
      <h1 className="text-3xl font-bold text-white mb-6 z-10 relative">🩺 ภาพรวมข้อมูล DMIS CKD ไตวาย</h1>
