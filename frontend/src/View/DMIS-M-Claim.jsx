...
import HsendSelect from '../components/util/HsendSelectForMophClaim'; // ใช้ HsendSelect สำหรับ DMIS M Claim
...
export default function DMIS_M_Claim() {
  ...
  const [list_hamin, setlist_hamin] = useState([]); // ✅ สร้าง state สำหรับ hmainOP
   const fetchAppointments = useCallback(async () => {
    ...
    try {
      ...
      // 3.4. ส่ง signal เข้าไปใน options ของ Axios/Fetch API
      // สมมติว่า API ของคุณ (API.DMIS_KTP.getClaimReportsV2) รองรับ options แบบนี้
      const options = { signal };

      //console.log("DEBUG: Request parameters sent to API:", requestParams);
      const response = await API.DMIS_M_CLIAM.getDMIS_MCLIAM_Summary(requestParams, options); // <-- ส่ง options ไปด้วย
      const responsedeatailData = await API.DMIS_M_CLIAM.getDMIS_MCLIAM_Detail(requestParams, options); // <-- ส่ง options ไปด้วย
      const ListNameType = await API.DMIS_M_CLIAM.getDMIS_MCLIAM_ListNameType(options);  // <-- ส่ง options ไปด้วย
      const list_hmainOP = await API.DMIS_M_CLIAM.getDMIS_MCLIAM_LlistHaminaOP(options); // <-- ส่ง options ไปด้วย
      ...
      if (!signal.aborted) {
        ...
        setInitialLoadDone(true); // ✅ เพิ่มตรงนี้
      }
...
useEffect(() => {
  fetchAppointments(); // โหลดข้อมูลทันทีเมื่อ component แรกเริ่ม mount
}, []); 
...
const resetFilters = () => {
  ...
  const handleHsendChange = (newSelectedHsend) => {
    setFilters(prev => ({ ...prev, Hsend: newSelectedHsend }));
  };
...
const handleAffiliationChange = (newSelectedAffiliation) => {
  setFilters(prev => ({
    ...
    name_Ampur: [] ,
     Hsend: []  // ✅ เคลียร์ค่าอำเภอทุกครั้งที่สังกัดเปลี่ยน
  }));
};
...
const handleAmpurChange = (newSelectedAmpur) => {
  ...
  return (
    <div className="p-6 bg-gradient-to-br from-blue-400 via-blue-100 to-blue-200 min-h-screen relative">
      ...
      <h1 className="text-3xl font-bold text-white mb-6 z-10 relative"> 📥 ภาพรวมข้อมูล DMIS MOPH CLAIM </h1>
...
/>
    ...
    {mode === 'area' && (
      <div className="grid md:grid-cols-3 gap-4">
        ...
        <AmpurSelect
          value={filters.name_Ampur}
          onChange={handleAmpurChange}
          typeAffiliations={filters.type_affiliation}
        />
        <HsendSelect
          value={filters.Hsend}
          onChange={handleHsendChange}
          typeAmpur={filters.name_Ampur}
          typeAffiliations={filters.type_affiliation}
        />
      </div>
    )}
