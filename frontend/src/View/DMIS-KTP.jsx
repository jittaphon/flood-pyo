...
import HsendSelect from '../components/HsendSelect';
...
export default function DMIS_KTP() {
  ...
  const [list_hamin, setlist_hamin] = useState([]); // ✅ สร้าง state สำหรับ hmainOP
   const fetchAppointments = useCallback(async () => {
    ...
    try {
      ...
      const response = await API.DMIS_KTP.getClaimReportsV2(requestParams, options); // <-- ส่ง options ไปด้วย
      const responsedeatailData = await API.DMIS_KTP.getClaimReportsDeatailV2(requestParams, options); // <-- ส่ง options ไปด้วย
      const ListNameType = await API.DMIS_KTP.getListNameType(options);  // <-- ส่ง options ไปด้วย
      const list_hmainOP = await API.DMIS_KTP.getlistHmainOP(options); // <-- ส่ง options ไปด้วย
...
const handleAmpurChange = (newSelectedAmpur) => {
  ...
  return (
    <div className="p-6 bg-gradient-to-br from-blue-400 via-blue-100 to-blue-200 min-h-screen relative">
      ...
      <h1 className="text-3xl font-bold text-white mb-6 z-10 relative"> 💳 ภาพรวมข้อมูล DMIS & KTP</h1>
